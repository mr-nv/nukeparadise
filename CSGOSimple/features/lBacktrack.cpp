...
#include "Logger.h"
void lBacktrack::OnMove(CUserCmd* pCmd) {
	...
	g_Lbot.settings = g_Config.lbot_items[pWeapon->m_Item().m_iItemDefinitionIndex()];

	if (!g_Lbot.settings.enabled) {
		data.clear();
		return;
	}
	...
	for (int i = 1; i <= g_GlobalVars->maxClients; ++i) {
		...
		if (!g_Lbot.settings.deathmatch && player->m_iTeamNum() == g_LocalPlayer->m_iTeamNum()) {
			if (data.count(i) > 0)
				data.erase(i);
			continue;
		}
	...
	for (auto& node : data) {
		...
		for (auto& bd : cur_data) {
			...
			if (std::fabsf(deltaTime) > g_Lbot.settings.backtrack.time)
				continue;

			Math::VectorAngles(bd.hitboxPos - localEyePos, angles);
			Math::FixAngles(angles);
			float fov = g_Lbot.GetFovToPlayer(pCmd->viewangles, angles);
			if (best_fov > fov) {
				best_fov = fov;
