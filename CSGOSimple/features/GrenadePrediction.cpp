...
void CCSGrenadeHint::Tick(int buttons)
{
	...
	if (!g_Config.esp_misc_grenade_prediction)
		return;
...
void CCSGrenadeHint::Paint()
{
	...
	if ((type) && act != ACT_NONE && path.size()>1 && weapon->IsGrenade())
	{
		Vector ab, cd;
		Vector prev = path[0];
		...
		{
			VGSHelper::Get().Draw3DCube(2.f, it->second, it->first, Color(0, 255, 0, 200));
		}

		VGSHelper::Get().Draw3DCube(2.f, OtherCollisions.rbegin()->second, OtherCollisions.rbegin()->first, Color(255, 0, 0, 200));
		...
		if (bestdmg > 0.f) {
			if (weap_id != WEAPON_HEGRENADE)
			{
				...
				{ 
					VGSHelper::Get().DrawText(firegrenade_didnt_hit ? "No collisions" : (EntName + " will be burnt.").c_str(), cd[0], cd[1] - 10, *BestColor, 13.5f);
				}
			}
			else
			{
				...
				{
					VGSHelper::Get().DrawText(("Most damage dealt to: " + EntName + " -" + std::to_string(bestdmg)).c_str(), cd[0], cd[1] - 10, *BestColor, 13.5f);
				}
			}
		}
	}
