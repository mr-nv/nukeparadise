...
{
	if (k_weapons_info.count(definition_index))
	{
		auto& icon_override_map = g_Config.skins.m_icon_overrides;
		const auto& original_item = k_weapons_info.at(definition_index);
	...
	const unsigned xuid_low) -> void
{
	...
	auto& icon_override_map = g_Config.skins.m_icon_overrides;
	if (config->definition_override_index && config->definition_override_index != definition_index && k_weapons_info.count(config->definition_override_index))
	{
		const auto old_definition_index = definition_index;
		definition_index = config->definition_override_index;
		const auto& replacement_item = k_weapons_info.at(config->definition_override_index);
		...
		if (old_definition_index && k_weapons_info.count(old_definition_index))
		{
			const auto& original_item = k_weapons_info.at(old_definition_index);
...
void Skinchanger::OnFrameStageNotify(ClientFrameStage_t stage)
{
		...
		return;
	{
		...
		const auto glove_config = &g_Config.skins.m_items[GLOVE_T_SIDE];
	...
	}
	{
		...
		for (int i = 0; weapons[i].IsValid(); i++)
		{
			...
			const auto active_conf = &g_Config.skins.m_items[is_knife(definition_index) ? WEAPON_KNIFE : definition_index];
